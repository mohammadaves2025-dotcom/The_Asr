...
const {
  categoryController, commentController, newsletterController,
...
// ─── Categories Router ────────────────────────────────────────────────────────
// Per permissions table: Manage categories = admin/superadmin only
const categoryRouter = express.Router();
categoryRouter.get('/', categoryController.getAll);
categoryRouter.get('/:slug', categoryController.getOne);
// Admin-only list (includes inactive)
categoryRouter.get('/admin/all',
  protect,
  authorize('admin', 'superadmin'),
  categoryController.adminGetAll
);
categoryRouter.post('/',
  protect,
  authorize('admin', 'superadmin'),
...
// ─── Comments Router (article-scoped) ────────────────────────────────────────
// Per permissions table: Moderate/delete comments = editor/admin/superadmin
...
commentRouter.patch('/:id/moderate',
  protect,
...
adminCommentRouter.patch('/:id/moderate',
  protect,
  authorize('editor', 'admin', 'superadmin'),
  commentController.moderate
);
adminCommentRouter.delete('/:id',
  protect,
  authorize('editor', 'admin', 'superadmin'),
  commentController.delete
);
...
// ─── Newsletter Router ────────────────────────────────────────────────────────
// Per permissions table: View newsletter subscribers = admin/superadmin only
...
newsletterRouter.get('/admin/subscribers',
  protect,
  authorize('admin', 'superadmin'),
  paginationValidator,
  newsletterController.adminList
);

// ─── Submissions Router ───────────────────────────────────────────────────────
// Per permissions table: View/manage submissions = editor/admin/superadmin
const submissionRouter = express.Router();
submissionRouter.post('/', submissionValidator, submissionController.create);
submissionRouter.get('/admin',
  protect,
  authorize('editor', 'admin', 'superadmin'),
  paginationValidator,
  submissionController.adminList
);
submissionRouter.patch('/admin/:id/status',
  protect,
  authorize('editor', 'admin', 'superadmin'),
...
// ─── Users Router ─────────────────────────────────────────────────────────────
// Per permissions table:
//   List & manage users     = admin/superadmin
//   Create contributor/editor accounts = admin/superadmin
//   Activate/deactivate     = admin/superadmin
//   Change any user's role  = superadmin only
//   Delete users            = superadmin only
//   Create admin accounts   = superadmin only (enforced in controller too)
//
// Static/prefixed routes MUST come before /:id to avoid Express treating
// "me" and "admin" as ObjectId values.
const userRouter = express.Router();

// Admin routes
userRouter.get('/admin/list',
  protect,
  authorize('admin', 'superadmin'),
  paginationValidator,
  userController.adminList
);
userRouter.post('/admin/create',
  protect,
  authorize('admin', 'superadmin'),
  userController.adminCreate           // controller double-checks: only superadmin creates admin/superadmin
);
userRouter.patch('/admin/:id/role',
  protect,
  authorize('superadmin'),             // Only superadmin can change roles
  userController.adminUpdateRole
);
userRouter.patch('/admin/:id/toggle-active',
  protect,
  authorize('admin', 'superadmin'),
  userController.adminToggleActive
);
userRouter.patch('/admin/:id/profile',
  protect,
  authorize('admin', 'superadmin'),
...
userRouter.delete('/admin/:id',
  protect,
  authorize('superadmin'),             // Only superadmin can delete users
...
// ─── Stats Router ─────────────────────────────────────────────────────────────
// Per permissions table: View stats dashboard = editor/admin/superadmin
  ...
  async (req, res, next) => {
    try {
      ...
      const [
        ...
        totalUsers, totalSubmissions, newSubmissions, viewsAgg, todayViewsAgg,
      ] = await Promise.all([
...
module.exports = {
  ...
  newsletterRouter, submissionRouter, userRouter, statsRouter,
};
