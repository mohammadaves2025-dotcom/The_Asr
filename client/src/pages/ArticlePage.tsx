...
import {
  Clock, Eye, Calendar, Link2, ArrowLeft, Bookmark, BookmarkCheck, AlertTriangle
} from 'lucide-react';
...
function ReadingProgress() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      ref.current.style.width = total > 0 ? `${Math.min((window.scrollY / total) * 100, 100)}%` : '0%';
    };
    ...
    <div className="border-t-2 border-ink pt-8 mt-10">
      ...
      <div className="flex items-start gap-4">
        {article.author?.avatar ? (
          <img src={article.author.avatar} alt={authorName} className="w-14 h-14 rounded-full object-cover flex-shrink-0" />
        ) : (
        ...
        <div className="container-site py-8 md:py-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-8">
              ...
              {article.subtitle && (
                <p className="text-lg md:text-xl text-ink-secondary font-sans leading-relaxed font-light mb-6">
                  {article.subtitle}
                </p>
              )}
                ...
                <div className="flex items-center gap-2.5">
                  {article.author?.avatar ? (
                    ...
                    <div className="w-9 h-9 rounded-full bg-brand-navy flex items-center justify-center flex-shrink-0">
                      <span className="text-brand-yellow font-bold text-[12px]">{authorName[0]}</span>
                    </div>
                  )}
                  <div>
                    {!article.isGuestAuthor && article.author?._id ? (
              ...
              {/* Newsletter CTA */}
              <div className="mt-10">
                <NewsletterInline source="article" variant="dark" />
              </div>
            ...
            <aside className="lg:col-span-4">
              <div className="sticky top-24 space-y-6">
                ...
                {/* Sidebar newsletter */}
                <NewsletterInline source="article-sidebar" variant="dark" />
              </div>
            </aside>
          </div>
        </div>
