...
function Avatar({ name, avatar, size = 8 }: { name: string; avatar?: string; size?: number }) {
  ...
  if (avatar) return <img src={avatar} alt={name} className={`${s} rounded-full object-cover flex-shrink-0`} />;
  return (
    <div className={`${s} rounded-full bg-brand-navy flex items-center justify-center flex-shrink-0`}>
      <span className="text-brand-yellow font-bold text-[10px]">{name?.[0] ?? '?'}</span>
    </div>
  );
}

function CommentItem({
  comment, articleId, depth = 0
}: { comment: Comment; articleId: string; depth?: number }) {
  ...
  const replyMutation = useMutation({
    mutationFn: (body: string) =>
      articlesService.addComment(articleId, { body, parentComment: comment._id }),
    onSuccess: () => {
  ...
  return (
    <div className={`${depth > 0 ? 'ml-8 border-l-2 border-gray-100 pl-4' : ''}`}>
      <div className="flex gap-3 py-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-2 mb-1">
            <span className="text-[13px] font-semibold text-ink font-sans">{comment.author?.name}</span>
            <span className="text-[10px] text-ink-muted font-sans">{formatRelative(comment.createdAt)}</span>
          </div>
          <p className="text-[14px] text-ink-secondary font-sans leading-relaxed">{comment.body}</p>
          ...
          {replyOpen && (
            <div className="mt-3">
              ...
              <div className="flex gap-2 mt-2">
                ...
                >
                  {replyMutation.isPending ? <Loader2 size={12} className="animate-spin" /> : 'Post Reply'}
                </button>
                <button
      ...
      {comment.replies?.map(reply => (
        <CommentItem key={reply._id} comment={reply} articleId={articleId} depth={depth + 1} />
      ))}
    </div>
  );
}
...
export default function CommentsSection({ articleId }: { articleId: string }) {
  ...
  const comments: Comment[] = (data?.data as any)?.data?.comments ?? [];

  const postMutation = useMutation({
    mutationFn: (body: string) => articlesService.addComment(articleId, { body }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['comments', articleId] });
      setCommentText('');
    },
  ...
  return (
    <section className="mt-12 pt-8 border-t-2 border-ink">
      ...
      {isAuthenticated ? (
        ...
        <div className="mb-8 bg-surface-secondary border border-gray-200 px-5 py-4 flex items-center justify-between gap-4">
          <p className="text-[13px] text-ink-secondary font-sans">
